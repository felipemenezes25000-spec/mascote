 ...
 * (toast + animação). Conecta simulação + moments reativos via creatureMoments.
...
export function useHomeBehavior({
  ...
  const buildCtx = (): BehaviorContext | null => {
    ...
    return {
      ...
      reduceMotion: useStore.getState().settings?.reduce_motion ?? false,
  ...
  const onEffect = (effect: BehaviorEffect) => {
