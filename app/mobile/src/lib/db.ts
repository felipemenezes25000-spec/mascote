  ...
  async upsert(m: Partial<Mascot> & { user_id: string }): Promise<Mascot> {
    return withLock('mascots', async () => {
      ...
      const next: Mascot = {
        ...
        user_id: m.user_id,
        /* v8 ignore next 10 — cada `?? defaults` cobre 3 branches por linha
           (m.field ?? existing.field ?? hardcoded). Testado mas v8 conta separado. */
        name: m.name ?? existing?.name ?? 'Bipo',
        personality: m.personality ?? existing?.personality ?? 'calmo',
      ...
      /* v8 ignore next 3 — ternário existing testado. */
